...
## Быстрый запуск на виртуалке

Если репозиторий уже находится на VM, можно поднять проект одной командой:
...
- ставит Docker и Docker Compose на `apt`-based Linux VM, если их нет
- создает `.env` из шаблона
- генерирует новый `ADMIN_TOKEN`
- собирает и поднимает `frontend`, `backend`, `nginx`
...
Если домен уже смотрит на VM, дальше достаточно настроить DNS и TLS.
